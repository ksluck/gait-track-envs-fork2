...
class HalfCheetahEnv(MujocoEnv, utils.EzPickle):
    def __init__(self, parametric=True, init_task=None):
        ...
        self.original_motor_gears = np.array([120.,90.,60.,120.,90.,30.])
        self.model_args = {"size": list(self.original_lengths), "mass": 14, "motor": list(self.original_motor_gears)}

        self.markers = ["thigh", "shin", "foot", "foottip"]
        self.legs = ["b", "f"]
        self.origin = "torso"
        print("Halfcheetah36 Motor Env")

        if parametric:
            MujocoEnv.__init__(self, 'parametric_half_cheetah_36_actions.xml', 5)
        else:
    ...
    def set_task(self, *task):
        ...
        self.model_args = {
        	...
        	"mass": 14.0 * (np.sum(self.current_lengths) / np.sum(self.original_lengths)),
        	"motor": list( 
        		self.original_motor_gears*0.75 + self.original_motor_gears*(self.original_lengths/self.current_lengths) * 0.25
        	),
        }
        self.build_model()
...
def register_half_cheetah_36_motor(env_name):
    if env_name == "GaitTrackHalfCheetah36Motor-v0":
        kwargs = {"parametric": True}
    elif env_name == "GaitTrackHalfCheetah36MotorOriginal-v0":
